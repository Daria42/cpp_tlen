...
//QPointF rotate(QPointF v, qreal a) {
//	QPointF point;
//	point.setX(v.x() * qCos(a) - v.y() * qSin(a));
//	point.setY(v.x() * qSin(a) + v.y() * qCos(a));
//	return point;
//}
...
void Edge::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *) {
	...
	QPointF A((from_->x() + to_->x()) / 2 + delta.x(), (from_->y() + to_->y()) / 2 + delta.y());
	QPointF B((from_->x() + to_->x()) / 2 - delta.x(), (from_->y() + to_->y()) / 2 - delta.y());
	setLine(QLineF(from_->pos(), to_->pos()));
	painter->setPen(pen());
	painter->drawLine(QLineF(from_->pos(), A));
	painter->drawLine(QLineF(B, to_->pos()));
	//qreal alpha = M_PI / 6;
	//QPointF left = 50 * rotate(-delta, alpha);
	//QPointF right = 50 * rotate(-delta, -alpha);
	//painter->drawLine(QLineF(to_->pos(), to_->pos() + left));
	//painter->drawLine(QLineF(to_->pos(), to_->pos() + right));
	QRectF rect((from_->x() + to_->x()) / 2 - 10, (from_->y() + to_->y()) / 2 - 10, 20, 20);
	QFont font;
