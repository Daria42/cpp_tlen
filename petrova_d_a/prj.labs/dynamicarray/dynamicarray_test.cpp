...
int main() {
	...
	catch(invalid_argument ex){ cout << "Нельзя создать массив отрицательной длины" << endl; }
	try { a[1000]; }
	catch (out_of_range ex) { cout << "Нельзя обратиться по индексу, превышающему длину массива" << endl; }
	try { a[-1]; }
	catch (out_of_range ex) { cout << "Нельзя обратиться по отрицательному индексу" << endl; }
	return 0;
}